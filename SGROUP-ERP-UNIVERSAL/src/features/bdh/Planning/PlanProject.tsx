...
import { Building, Save, LayoutGrid } from 'lucide-react-native';
...
export const PlanProject = () => {
  ...
  const { data, isLoading } = useGetExecPlan(year, 'BASE', 'PROJECT');
  const upsertPlan = useUpsertExecPlan();
  const { colors, isDark } = useAppTheme();

  const handleSave = () => {
    upsertPlan.mutate({
      year, scenario: 'BASE', tab: 'PROJECT',
      ...
      <View style={[styles.header, { borderBottomColor: colors.border }]}>
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: spacing.md }}>
            ...
            <View>
              <Text style={[typography.hero, { color: colors.text }]}>Project Timeline {year}</Text>
              <Text style={[typography.body, { color: colors.warning }]}>Milestones & Cash Out</Text>
            </View>
          </View>
