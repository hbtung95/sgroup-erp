...
import { Calculator, Save, LayoutGrid } from 'lucide-react-native';
...
export const PlanFinance = () => {
  ...
  const { data, isLoading } = useGetExecPlan(year, 'BASE', 'FINANCE');
  const upsertPlan = useUpsertExecPlan();
  const { colors, isDark } = useAppTheme();

  const handleSave = () => {
    upsertPlan.mutate({
      year, scenario: 'BASE', tab: 'FINANCE',
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: spacing.md }}>
            <View style={[styles.iconBox, { backgroundColor: isDark ? 'rgba(34, 197, 94, 0.15)' : 'rgba(21, 128, 61, 0.1)' }]}>
              <Calculator color={colors.success} size={32} strokeWidth={1.5} />
            </View>
            <View>
              <Text style={[typography.hero, { color: colors.text }]}>Financial Planning {year}</Text>
              <Text style={[typography.body, { color: colors.success }]}>Budget & Forecast</Text>
            </View>
          </View>
