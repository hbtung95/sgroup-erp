...
import { PieChart, Save, LayoutGrid } from 'lucide-react-native';
...
export const PlanMarketing = () => {
  ...
  const { data, isLoading } = useGetExecPlan(year, 'BASE', 'MK');
  const upsertPlan = useUpsertExecPlan();
  const { colors, isDark } = useAppTheme();

  const handleSave = () => {
    upsertPlan.mutate({
      year, scenario: 'BASE', tab: 'MK',
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: spacing.md }}>
            <View style={[styles.iconBox, { backgroundColor: isDark ? 'rgba(6, 182, 212, 0.15)' : 'rgba(8, 145, 178, 0.1)' }]}>
              <PieChart color={colors.accentCyan} size={32} strokeWidth={1.5} />
            </View>
            <View>
              <Text style={[typography.hero, { color: colors.text }]}>Marketing Budget {year}</Text>
              <Text style={[typography.body, { color: colors.accentCyan }]}>Spend & ROI Targets</Text>
            </View>
          </View>
