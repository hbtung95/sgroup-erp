...
import { Briefcase, Save, LayoutGrid } from 'lucide-react-native';
...
export const PlanAgency = () => {
  ...
  const { data, isLoading } = useGetExecPlan(year, 'BASE', 'AGENCY');
  const upsertPlan = useUpsertExecPlan();
  const { colors, isDark } = useAppTheme();

  const handleSave = () => {
    upsertPlan.mutate({
      year, scenario: 'BASE', tab: 'AGENCY',
      ...
      <View style={[styles.header, { borderBottomColor: colors.border }]}>
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: spacing.md }}>
            ...
            <View>
              <Text style={[typography.hero, { color: colors.text }]}>Agency Plan {year}</Text>
              <Text style={[typography.body, { color: colors.purple }]}>Expansion Goals</Text>
            </View>
          </View>
