...
import { UserCog, Save, LayoutGrid } from 'lucide-react-native';
...
export const PlanHR = () => {
  ...
  const { data, isLoading } = useGetExecPlan(year, 'BASE', 'HR');
  const upsertPlan = useUpsertExecPlan();
  const { colors, isDark } = useAppTheme();

  const handleSave = () => {
    upsertPlan.mutate({
      year, scenario: 'BASE', tab: 'HR',
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: spacing.md }}>
            <View style={[styles.iconBox, { backgroundColor: isDark ? 'rgba(14, 165, 233, 0.15)' : 'rgba(2, 132, 199, 0.1)' }]}>
              <UserCog color={colors.brand} size={32} strokeWidth={1.5} />
            </View>
            <View>
              <Text style={[typography.hero, { color: colors.text }]}>HR Strategy {year}</Text>
              <Text style={[typography.body, { color: colors.brand }]}>Headcount & Payroll</Text>
            </View>
          </View>
