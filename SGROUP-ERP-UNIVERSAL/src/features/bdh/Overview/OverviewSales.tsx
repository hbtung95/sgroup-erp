...
import { ShoppingCart, Activity } from 'lucide-react-native';
      ...
      <View style={[styles.header, { borderBottomColor: colors.border }]}>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: spacing.md }}>
          <View style={[styles.iconBox, { backgroundColor: isDark ? 'rgba(245, 158, 11, 0.15)' : 'rgba(217, 119, 6, 0.1)' }]}>
            <ShoppingCart color={colors.warning} size={32} strokeWidth={1.5} />
          </View>
          <View>
            <Text style={[typography.hero, { color: colors.text }]}>Sales Sector Pulse</Text>
            <Text style={[typography.body, { color: colors.warning }]}>CRM & Pipeline Overview</Text>
          </View>
        </View>
      </View>
      ...
      <View style={styles.content}>
        <SGSection style={styles.pulseCard}>
          <SGEmptyState
            ...
            subtitle={`Sales data stream is operational: ${data?.status || 'Active'}\nWaiting for real-time telemetry from CRM engine.`}
          />
        </SGSection>
      </View>
    </View>
