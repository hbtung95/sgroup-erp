      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 pb-10">
              ...
              {projects.length === 0 ? (
                ...
                <div className="flex flex-col gap-1 relative z-10">
                  {projects.slice(0, 5).map((proj, idx) => {
                    ...
                    return (
                      <Link to={`/ProjectModule/board?id=${proj.id}`} key={proj.id} className="flex items-center gap-5 p-4 sm:p-5 rounded-[24px] hover:bg-white/50 dark:hover:bg-white/5 transition-all group relative overflow-hidden">
        ...
        <div className="flex flex-col gap-8 sg-stagger" style={{ animationDelay: '500ms' }}>
          ...
          {recentLegalDocs.length === 0 ? (
            <div className="bg-white dark:bg-black/30 backdrop-blur-[32px] border border-slate-200/80 dark:border-white/5 rounded-[32px] p-12 text-center shadow-md">
              <div className="w-16 h-16 rounded-full bg-sg-btn-bg flex items-center justify-center mx-auto mb-4 border border-sg-border">
            ...
            <div className="flex flex-col gap-4">
              {recentLegalDocs.map(doc => {
                ...
                return (
                  <div key={doc.id} className={`bg-slate-50 dark:bg-black/40 backdrop-blur-3xl border border-slate-200/80 dark:border-white/5 rounded-[24px] p-5 flex flex-col gap-3 shadow-sm hover:shadow-lg hover:-translate-y-1.5 hover:bg-slate-100 dark:hover:bg-black/60 transition-all cursor-pointer relative overflow-hidden group`}>
