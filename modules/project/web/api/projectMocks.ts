import type { Project, Product, LegalDoc, DashboardStats, StatusCount, ApiResponse } from "@sgroup/types";

// Form payloads used only by mock API
type CreateProjectForm = Partial<Project>;
type CreateProductForm = Partial<Product>;
...
export async function listProjects(params?: { page?: number; limit?: number; search?: string }): Promise<ApiResponse<Project[]>> {
  await delay(300);
  return { success: true as const, data: MOCK_PROJECTS, meta: { total: MOCK_PROJECTS.length, page: 1, page_size: 10, total_pages: 1 } };
}
...
export async function listProducts(projectId: string, params?: {
  ...
  return { success: true as const, data, meta: { total: data.length, page: 1, page_size: 1000, total_pages: 1 } };
}
