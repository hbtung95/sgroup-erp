...
import { useDashboardKPIs, usePipelineSummary, useTopSellers, useTeamPerformance, formatVND } from '../../hooks/useSalesData';
import { SkeletonKPICard, SkeletonChart } from '../../components/shared';
...
export function DirectorDashboard() {
  ...
  const personalKpi = useMemo(() => {
    if (!kpi) return null;
    return {
      revenue: kpi.revenue / 10,
      pipelineValue: kpi.pipelineValue / 10,
      totalLeads: Math.round(kpi.totalLeads / 10),
      pendingApprovals: 5, // fake personal
    };
  }, [kpi]);

  const sortedTeams = [...(topTeams || [])].sort((a, b) => b.gmv - a.gmv).slice(0, 5);
  ...
  return (
    <div className="flex-1 overflow-y-auto p-6 lg:p-8 space-y-6">
      ...
      {kpiLoading ? (
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <KPICard
            icon={<Target size={20} />} label="VND Doanh Thu Công Ty"
          ...
          />
          <KPICard
            icon={<PieChart size={20} />} label="Pipeline Toàn Công Ty"
      ...
      {personalKpi && !kpiLoading && (
        <div className="bg-white dark:bg-black/30 backdrop-blur-3xl rounded-[28px] border border-slate-200/80 dark:border-sg-border p-5 shadow-sg-md sg-stagger" style={{ animationDelay: '200ms' }}>
          <h3 className="text-[14px] font-black text-sg-heading mb-4 flex items-center gap-2">
            <TrendingUp size={16} className="text-emerald-500" /> Chỉ Số Cá Nhân Của Bạn
          </h3>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <div>
        ...
        <div className="bg-white dark:bg-black/30 backdrop-blur-3xl rounded-[28px] border border-slate-200/80 dark:border-sg-border p-6 shadow-sg-md sg-stagger flex flex-col h-full" style={{ animationDelay: '300ms' }}>
          <div className="flex items-center justify-between mb-5">
            <div className="flex items-center gap-3">
              ...
              <h3 className="text-[15px] font-black text-sg-heading">Bảng Xếp Hạng Phòng Ban</h3>
            </div>
            <span className="text-[11px] font-bold px-2 py-1 bg-sg-card rounded border border-sg-border">Top Nhóm Xuất Sắc</span>
          </div>
          ...
          <div className="flex-1 space-y-3 overflow-y-auto pr-2">
            {teamsLoading ? (
              ...
              (sortedTeams || []).map((team, i) => (
                <div key={team.teamId} className="p-3 rounded-xl border border-sg-border bg-sg-card/30 flex items-center justify-between group overflow-hidden hover:border-orange-500/30 transition-all">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-sg-muted uppercase tracking-wider flex items-center gap-1">
                         <Building2 size={10} /> {team.staffCount} Nhân sự • {team.closedDeals} Deals
                      </p>
                    </div>
                  </div>
                  <div className="text-right">
                    <span className="text-[13px] block font-black text-emerald-500">{formatVND(team.gmv)}</span>
                    <span className="text-[10px] block font-semibold text-sg-muted">GMV Phòng</span>
                  </div>
                </div>
              ))
            )}
        ...
        {pipelineLoading ? (
          ...
          <div className="bg-white dark:bg-black/30 backdrop-blur-3xl rounded-[28px] border border-slate-200/80 dark:border-sg-border p-6 shadow-sg-md sg-stagger flex flex-col h-full" style={{ animationDelay: '400ms' }}>
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h3 className="text-[15px] font-black text-sg-heading">Phễu Bán Hàng Toàn Công Ty</h3>
