...
export function SGStatsCard({
  ...
  return (
    <div
      onClick={onClick}
      className={`border border-sg-border p-6 rounded-[24px] shadow-sm flex flex-col ${resolvedVariant.bg} ${onClick ? 'cursor-pointer hover:shadow-md hover:-translate-y-0.5 transition-all' : ''} ${className}`}
    >
      <div className="flex flex-row items-center gap-3 mb-4">
        <div className={`w-11 h-11 rounded-xl flex items-center justify-center shrink-0 ${resolvedVariant.iconBg}`}>
          <Icon size={20} className={resolvedVariant.iconColor} />
        </div>
      ...
      <div className="flex items-baseline gap-1.5 mt-auto">
        ...
        {unit ? <span className="text-[14px] font-bold text-sg-subtext shrink-0">{unit}</span> : null}
      </div>
    </div>
  );
}
