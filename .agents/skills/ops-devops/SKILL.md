...
| Layer | Platform | URL |
|-------|----------|-----|
| **Frontend** | Vercel | `sgroup-erp.vercel.app` |
| **Backend (staging)** | Render | `sgroup-erp-api.onrender.com` |
| **Backend (prod)** | Fly.io | `sgroup-erp-api.fly.dev` |
...
```env
# Vercel (frontend)
NEXT_PUBLIC_API_BASE_URL=https://sgroup-erp-api.fly.dev
...
jobs:
  ...
  build-image:
    steps:
      - docker build -t eng-backend:$SHA .
      - docker push registry/eng-backend:$SHA
