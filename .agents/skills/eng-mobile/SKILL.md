...
| GitHub Actions, CI pipeline | → **ops-devops** |
| Storage, sync, conflict resolution | → **eng-mobile** |
| API design/backend integration | → **eng-backend-go** + **solution-architect-vct** |
| Security (biometrics, storage) | → **qa-security** |
| UI/UX design review | → **arch-ui-ux** |
| Release management | → **ops-release-manager** |
| Store compliance | → **ops-release-manager** |
...
DELEGATE:
  □ Build configuration → ops-devops
  □ Test strategy → qa-engineer
  □ Performance deep-dives → qa-performance
  □ CI/CD pipeline → ops-devops
  □ Offline architecture → eng-mobile
```
...
| Situation | Consult |
|---|---|
| Shared component design | → **arch-ui-ux** + **eng-tech-lead** |
| API design for mobile | → **solution-architect-vct** for offline-friendly patterns |
| Mobile CI/CD | → **ops-devops** for EAS pipeline |
| Push notification setup | → **ops-devops** for server config |
| App store compliance | → **ops-release-manager** for submission |
| Mobile security | → **qa-security** for biometrics/storage |
...
| Offline architecture | → **eng-mobile** for sync patterns |
