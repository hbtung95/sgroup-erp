...
|---|---|---|---|
| 🏗️ SA | Solution Architect | `arch-solution` | Architecture, API design, database schema |
| 📋 BA | Business Analyst | `domain-core` | Requirements, domain modeling, regulations |
| 🎯 PO | Product Owner | `meta-product-manager` | Prioritization, backlog, release planning |
| 🔧 CTO | Chief Technology Officer | `meta-cto` | Code quality, standards, tech strategy |
| 📅 PM | Project Manager | `meta-product-manager` | Sprint planning, risk, progress tracking |

### Specialist Tier (Technical)
| Code | Role | Skill | Focus |
|---|---|---|---|
| 🧪 QA | QA Engineer | `qa-engineer` | Test automation, E2E, regression, quality metrics |
| 🎨 UXD | UX Designer | `arch-ui-ux` | User flows, wireframes, accessibility, design system |
| ⚙️ DevOps | DevOps / SRE | `ops-devops` | CI/CD, Docker/K8s, monitoring, deployment |
| 🗄️ DBA | Database Administrator | `arch-database` | Query optimization, migrations, PostgreSQL tuning |
| 🔒 SecEng | Security Engineer | `qa-security` | OWASP, secrets management, vulnerability scanning |
| 👨‍💻 TL | Tech Lead | `eng-tech-lead` | Deep code review, patterns, debugging, mentoring |
| 📝 TW | Technical Writer | `ops-tech-writer` | API docs, guides, changelogs, onboarding |
| 📊 DA | Data Analyst | `data-analyst` | KPIs, dashboards, rating algorithms, analytics |

### Process Tier (Agile & Release)
| Code | Role | Skill | Focus |
|---|---|---|---|
| 🔄 SM | Scrum Master | `meta-product-manager` | Agile ceremonies, impediments, team health |
| 📦 RM | Release Manager | `ops-release-manager` | Versioning, deployment coordination, rollback |

### Meta Tier (Self-Improvement)
| Code | Role | Skill | Focus |
|---|---|---|---|
| 🔎 AUD | Project Auditor | `meta-cto` | 12-point project health audit, findings tracking |
| 🧬 EVO | Skill Evolver | `meta-skill-evolver` | Technology monitoring, skill upgrades, freshness |

### Domain & Operations Tier
| Code | Role | Skill | Focus |
|---|---|---|---|
| 🌐 i18n | i18n Manager | `i18n-manager` | Translations, locale formatting, terminology |
| 📱 MOB | Mobile Lead | `eng-mobile` | Expo/React Native, offline-first, EAS builds |
...
| 🚨 INC | Incident Manager | `ops-release-manager` | SEV response, runbooks, post-mortems, SLOs |

### Execution Skills (Implementation)
| Skill | Focus |
|---|---|
| `eng-backend-go` | Go 1.26 backend — Clean Architecture, 25+ modules, 85 migrations |
| `eng-frontend` | Next.js 16 App Router, React 19, TailwindCSS 4, Zustand 5 |
| `arch-ui-ux` | Design system — 59 components, design tokens, 22 admin pages |
| `ops-devops` | Expo EAS Build, app signing, OTA updates, Metro bundler |
| `qa-engineer` | Jest, Maestro E2E, device matrix, accessibility testing |
| `qa-performance` | Hermes profiling, FlatList, bundle analysis, battery |
| `ops-devops` | GitHub Actions + EAS CI, automated store submission |
| `eng-mobile` | AsyncStorage/MMKV, sync queue, conflict resolution, offline scoring |
...
| **Mobile Feature** | Mobile-specific features | MOB → SA → QA | ops-devops, qa-engineer |
...
Phase 4: IMPLEMENT
├── Use eng-backend-go skill for backend code
├── Use eng-frontend skill for frontend code
└── Use arch-ui-ux for new UI components
...
✓ CSS variable tokens (--sgroup-*), no Tailwind dark: modifier
